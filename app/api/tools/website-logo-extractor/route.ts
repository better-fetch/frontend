...
import {
  extractWebsiteLogos,
  WEBSITE_LOGO_EXTRACTOR_INPUT_SCHEMA,
  type WebsiteLogoExtractorFetchResult,
...
export async function POST(request: NextRequest) {
  ...
  const parsed = WEBSITE_LOGO_EXTRACTOR_INPUT_SCHEMA.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: key, error: insertError } = await admin
    ...
    .insert({
      user_id: user.id,
      name: "Dashboard: Website Logo Extractor",
      key_hash: hash,
      key_prefix: prefix,
    })
    .select("id")
  ...
  if (insertError || !key) {
    console.error("dashboard logo tool key insert failed:", insertError);
    return NextResponse.json({ error: "could not start tool run" }, { status: 500 });
  }

  try {
    const extraction = await extractWebsiteLogos(parsed.data, async (resource) => {
      const response = await fetch(`${API_BASE}/v1/fetch`, {
        ...
        body: JSON.stringify({
          ...
          include_html: resource.responseKind === "html",
          extra_headers:
            resource.responseKind === "json"
    ...
    const { error } = await admin
      ...
      .eq("id", key.id)
      .eq("user_id", user.id);
    if (error) console.error("dashboard logo tool key revoke failed:", error);
  }
}
...
async function parseFetchResponse(
  ...
  return parsed as WebsiteLogoExtractorFetchResult;
}
