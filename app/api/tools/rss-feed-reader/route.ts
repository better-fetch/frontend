...
import {
  readRssFeeds,
  RSS_FEED_READER_INPUT_SCHEMA,
  type RssFeedReaderFetchResult,
...
export async function POST(request: NextRequest) {
  ...
  const parsed = RSS_FEED_READER_INPUT_SCHEMA.safeParse(body);
  if (!parsed.success) {
  ...
  const { data: key, error: insertError } = await admin
    ...
    .insert({
      user_id: user.id,
      name: "Dashboard: RSS Feed Reader",
      key_hash: hash,
      key_prefix: prefix,
    })
    .select("id")
  ...
  if (insertError || !key) {
    console.error("dashboard rss feed tool key insert failed:", insertError);
    return NextResponse.json({ error: "could not start tool run" }, { status: 500 });
  }

  try {
    const extraction = await readRssFeeds(parsed.data, async (resource) => {
    ...
    const { error } = await admin
      ...
      .eq("id", key.id)
      .eq("user_id", user.id);
    if (error) console.error("dashboard rss feed tool key revoke failed:", error);
  }
}

async function parseFetchResponse(response: Response): Promise<RssFeedReaderFetchResult> {
  ...
  return parsed as RssFeedReaderFetchResult;
}
